...
#define DUMP_TEMPLATE(code_ptr, tmp_ptr, name, size, written, read, jtargetpos)\
{\
...
#define FIRST_POSTLUDE(code_ptr,tmp,postlude_sz,resid_params,ret_value, dsp) { \
    DEBUG_PRINT("FIRST POSTLUDE %p..%p >-> %p..", tmp, tmp+postlude_sz, code_ptr);\
    FIRST_POSTLUDE_EXTRA(code_ptr,tmp,postlude_sz,resid_params,ret_value, dsp) \
...
#define REST_POSTLUDE(code_ptr,tmp,postlude_sz,resid_params,ret_value,dsp) {\
 DEBUG_PRINT("POSTLUDE %p..%p >-> %p..", tmp, tmp+postlude_sz, code_ptr);\
 REST_POSTLUDE_EXTRA(code_ptr,tmp,postlude_sz,resid_params,ret_value,dsp)\
 memcpy(code_ptr, tmp, postlude_sz);\
 code_ptr += postlude_sz;\
 DEBUG_PRINT("%p\n", code_ptr);\
} \
